...
bool write_to_influxdb(float temperature, float humidity, const char *source) {
    ...
    if (res == CURLE_OK) {
        ...
        if (response_code == 204) {
            ...
            log_message(LOG_INFO, "Successfully wrote %s data to InfluxDB", source);
        } else {
...
// Function to parse a Read Input Registers response
void parse_read_registers_response(unsigned char *buffer, int length, float *temperature, float *humidity) {
...
bool read_temperature_humidity_rtu(int serial_fd, int gpio_pin, float *temperature, float *humidity) {
    ...
    if (send_read_input_registers(serial_fd, gpio_pin, REG_TEMPERATURE, 2) != 0) {
        log_message(LOG_ERROR, "Failed to send Modbus request");
        return false;
    }
    ...
    if (bytes_read <= 0) {
        log_message(LOG_ERROR, "No response received from Modbus slave");
        return false;
    }
    
    // Debug output
    log_message(LOG_DEBUG, "Received response (%d bytes):", bytes_read);
    if (current_log_level >= LOG_DEBUG) {
        print_hex_buffer(buffer, bytes_read);
    }
    
    // Parse the response
    parse_read_registers_response(buffer, bytes_read, temperature, humidity);
    ...
    if (select_result > 0 && FD_ISSET(can_socket, &read_fds)) {
        ...
        if (nbytes > 0) {
            ...
            if (frame.can_id == TARGET_CAN_ID) {
                log_message(LOG_DEBUG, "Received CAN frame with ID 0x%X", frame.can_id);
                
                return extract_can_data(&frame, temperature, humidity);
            } else {
                log_message(LOG_DEBUG, "Ignored CAN frame with ID 0x%X (not matching target 0x%X)", 
...
void print_statistics() {
    ...
    log_message(LOG_INFO, "Last RTU Temperature: %.2f°C", last_rtu_temperature);
    log_message(LOG_INFO, "Last RTU Humidity: %.2f%%", last_rtu_humidity);
    log_message(LOG_INFO, "Last CAN Temperature: %.2f°C", last_can_temperature);
    log_message(LOG_INFO, "Last CAN Humidity: %.2f%%", last_can_humidity);
    ...
    if (last_rtu_read > 0) {
        ...
        log_message(LOG_INFO, "Last Successful RTU Read: %s", rtu_time_buffer);
    }
    
    if (last_can_read > 0) {
        tm_info = localtime(&last_can_read);
        strftime(can_time_buffer, 30, "%Y-%m-%d %H:%M:%S", tm_info);
        log_message(LOG_INFO, "Last Successful CAN Read: %s", can_time_buffer);
    }
}
...
int main(void) {
    ...
    float can_temperature, can_humidity;
    
    // Set up signal handling for graceful termination
    signal(SIGINT, handle_signal);
    signal(SIGTERM, handle_signal);
    
    log_message(LOG_INFO, "Starting Combined ESP32 Modbus RTU and CAN Monitor...");
    ...
    while (running) {
        ...
        // Read temperature and humidity via CAN bus
        if (read_temperature_humidity_can(can_socket, &can_temperature, &can_humidity)) {
            // Update last successful values
            last_can_temperature = can_temperature;
            last_can_humidity = can_humidity;
            time(&last_can_read);
            
            // Write to InfluxDB with source tag "CAN"
            if (write_to_influxdb(can_temperature, can_humidity, "CAN")) {
                influx_writes++;
            } else {
                error_count++;
            }
